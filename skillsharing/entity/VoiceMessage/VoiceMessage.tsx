...
const VoiceMessage: React.FC<PropType> = ({ message, isSelected, isDecrypted }) => {
    ...
    useEffect(() => {
        ...
        if (isPlayingMessage && message.message_id === voiceMessageId) {
            const messageTimer = document.getElementById(
                `voice-message-${message.message_id}-duration`,
            ) as HTMLElement;
            const messageRangeBar = document.getElementById(
                `voice-range-bar-${message.message_id}`,
            ) as HTMLInputElement;
            ...
            deleteIndex = setInterval(() => {
                ...
                if (messageTimer !== null) {
                    messageTimer.innerHTML = FormatMinutesSecondDuration(
                        player.currentTime * SECOND_IN_MILLISECONDS,
                    );
                }
                ...
                >
                    <div className="chat-voice-message-content">
                        ...
                        <img
                            ...
                            src={
                                '/shared/' +
                                (isPlayingMessage && voiceMessageId === message.message_id
                                    ? 'pause'
                                    : 'play-button') +
                                '_white.png'
                            }
                            alt="Включить голосовое"
                            onClick={(event) => {
                                event.stopPropagation();
                        ...
                        {voiceMessageId === message.message_id && (
                            <RangeBar
                                id={`voice-range-bar-${message.message_id}`}
                                rangeClassName="voice-range-bar"
                                min={0}
                                max={100}
                                step={1}
                                value={
                                    (player.currentTime / message.voice_duration!) *
                                    100 *
